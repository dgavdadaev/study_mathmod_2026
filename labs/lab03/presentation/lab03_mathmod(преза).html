...
1. Изучить три варианта моделей Ланчестера.  
2. Построить графики, отражающие изменение численности противоборствующих сторон.  
3. Проанализировать динамику процессов и определить сторону-победителя.  

# Теория: модели боевых действий

## Общая идея моделей Ланчестера

В модели рассматриваются две противоборствующие стороны, численность которых описывается функциями

$$
x(t), \quad y(t)
$$

Если в некоторый момент времени одна из функций обращается в ноль, то соответствующая сторона считается проигравшей.
...
1. Линейная модель демонстрирует плавное и предсказуемое уменьшение обеих переменных.  
2. В нелинейной модели значение $y(t)$ практически сразу стремится к нулю.  
3. Параметры существенно определяют скорость затухания решений.  
4. В линейной системе влияние параметров выражено более отчётливо.  
5. Нелинейная модель требует больше вычислительных ресурсов, но при этом остаётся относительно простой в вычислительном плане.  
6. Метрика $\text{norm\_final}$ снижается при увеличении параметра, что свидетельствует об усилении затухания системы.  

...
# взаимодействия хищников и жертв с помощью модели Лотки-Вольтерры.
# 
# ## Теоретическое введение
# 
# Модель Лотки-Вольтерры (хищник-жертва) описывается системой уравнений:
# 
# $$
# \begin{cases}
...
# - $\delta$ - коэффициент конверсии пищи в потомство хищников
...
# 
# Исследуем 6 различных сценариев, меняя ключевые параметры:
# 1. **Базовый сценарий** - классические параметры (α=0.1, β=0.02, δ=0.01, γ=0.3)
# 2. **Высокая рождаемость жертв** - увеличенное α (α=0.2)
# 3. **Высокое поедание** - увеличенное β (β=0.04)
# 4. **Высокая конверсия** - увеличенное δ (δ=0.02)
# 5. **Высокая смертность хищников** - увеличенное γ (γ=0.5)
# 6. **Низкая смертность хищников** - уменьшенное γ (γ=0.1)

# Формат: [α, β, δ, γ, название_сценария]
parameter_sets = [
...
# 
# ### 1. Сравнение динамики жертв для всех сценариев
...
# ### 2. Сравнение динамики хищников для всех сценариев
...
# ### 4. Детальный анализ базового сценария

base_scenario = "Базовый сценарий"
df_base = all_results[base_scenario]

# Получаем параметры базового сценария
...
# Стационарные точки для базового сценария
x_star_base = γ_base / δ_base
y_star_base = α_base / β_base

# Спектральный анализ для базового сценария
...
# ### 5. Сравнение амплитуд колебаний
...
# ### 6. Сравнение средних значений со стационарными точками
